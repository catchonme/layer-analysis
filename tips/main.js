;!function(window, undefined) {
  ...
  var layer = {
    ...
    prompt: function(content, options, yes){
      ...
      switch (contentType) {
        ...
        case 'string' :
          {
            var optionsType = typeof options == 'function';
            if (optionsType) {
              yes = options;
              options = {};
            }
  ...
  Class.pt.buildUI = function() {
    ...
    var closeHTML = "";
    if (config.closeBtn) {
      closeHTML =  '<span class="layui-layer-setwin">'
    ...
    var contentHTML = "";
    // 匹配prompt的时候
    if (config.contentType && config.contentType == 'input') {
      // typeof (config.content) === 'string' ? '' : config.content = '';
      contentHTML = '<div class="layui-layer-prompt layui-layer-content">'
                  +   '<input class="layui-layer-input" value="'+(config.content||'')+'"/>';
                  + '</div>'
      // 其他时候正常显示内容
    } else {
      contentHTML =
      ...
      layero.find('.layui-layer-btn').children('a').on('click', function(event) {
        ...
        if (event.target.className == 'layui-layer-btn0') {
          if (config.yes) {
            // 兼容prompt
            if (config.contentType && config.contentType == 'input') {
              var prompt = layero.find('.layui-layer-input').val();
              config.yes(prompt);
            } else {
              config.yes();
            }
